...
poster_empty, fanart_empty, clear_property = kodi_utils.empty_poster, kodi_utils.addon_fanart, kodi_utils.clear_property
...
fen_str, addon_icon, database, maincache_db = ls(32036), kodi_utils.addon_icon, kodi_utils.database, kodi_utils.maincache_db
...
def movie_sets_to_collection_choice(collection_id):
	...
	data = movie_set_meta(collection_id, metadata_user_info())
	...
	if movies_added:
		...
		if 'existing' in result and result['existing']['movies'] == movies_to_add: ok_dialog(text='All Movies Already in Trakt Collection', top_space=True)
		else: ok_dialog(text='Error. No Movies Added to Trakt Collection', top_space=True)
		...
		for i in start_list:
			try:
				if page_ref == 0: line1 = '%s %s' % (ls(32022), str(i))
					...
					if i == current_page: line1 = '[COLOR %s][B]%s   |   %s[/B][/COLOR]' % (get_property('fen.highlight'), line1, current_page_str)
			except: line1 = ''
...
def playback_choice(media_type, poster, meta, season, episode, window_xml):
	...
	if choice == 'clear_and_rescrape':
		...
		if media_type in ('movie', 'movies'): play_params = {'mode': 'play_media', 'media_type': 'movie', 'tmdb_id': meta['tmdb_id'],
						'custom_title': custom_title, 'prescrape': 'false', 'autoplay': 'false'}
		...
		if media_type in ('movie', 'movies'): play_params = {'mode': 'play_media', 'media_type': 'movie', 'tmdb_id': meta['tmdb_id'], 'prescrape': 'false', 'autoplay': 'false'}
		else: play_params = {'mode': 'play_media', 'media_type': 'episode', 'tmdb_id': meta['tmdb_id'], 'season': season, 'episode': episode, 'prescrape': 'false', 'autoplay': 'false'}
		if aliases:
			poster_main, poster_backup = get_art_provider()[0:2]
			poster = meta.get('custom_poster') or meta.get(poster_main) or meta.get(poster_backup) or poster_empty
			list_items = [{'line1': i, 'icon': poster} for i in aliases]
...
def highlight_choice():
	...
	if chosen_color:
		...
		set_property('fen.highlight', chosen_color)
...
def options_menu(params, meta=None):
	...
	if get_property('fen.custom_context_menu') == 'true' and not from_extras:
	...
	if choice == None: return
		...
		close_all_dialog()
		return run_plugin({'mode': 'random_choice', 'tmdb_id': tmdb_id, 'poster': poster})
