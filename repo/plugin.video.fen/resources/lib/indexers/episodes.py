...
get_property, poster_empty, fanart_empty = kodi_utils.get_property, kodi_utils.empty_poster, kodi_utils.addon_fanart
...
def build_episode_list(params):
	...
	if not get_property('fen_cancel_content_build') == 'true':
		try:
			...
			meta_user_info, watched_indicators = metadata_user_info(), watched_indicators_info()
			watched_info, show_unaired,  = get_watched_info(watched_indicators), show_unaired_info(), 
			fanart_enabled = meta_user_info['extra_fanart_enabled']
			hide_watched = is_widget and meta_user_info['widget_hide_watched']
			current_date, adjust_hours, bookmarks = get_datetime_function(), date_offset_info(), get_bookmarks(watched_indicators, 'episode')
			...
			if all_episodes: episodes_data = all_episodes_meta_function(meta, meta_user_info, show_specials())
