# -*- coding: utf-8 -*-
from metadata import movie_meta, movie_set_meta
from modules import kodi_utils, settings
from modules.meta_lists import oscar_winners
from modules.utils import manual_function_import, get_datetime, make_thread_list_enumerate, adjust_premiered_date
...
poster_empty, fanart_empty, get_property = kodi_utils.empty_poster, kodi_utils.addon_fanart, kodi_utils.get_property
...
class Movies:
	...
	def fetch_list(self):
		...
		if not get_property('fen_cancel_content_build') == 'true':
			try:
				...
				if self.action in tmdb_main:
					...
					data = sorted(movie_set_meta(self.params_get('tmdb_id'), metadata_user_info())['parts'], key=lambda k: k['release_date'] or '2050')
	...
	def worker(self):
		...
		self.watched_title = trakt_str if self.watched_indicators == 1 else fen_str
	...
	def build_movie_sets_content(self, item_position, _id):
		try:
			cm = []
			cm_append = cm.append
			meta = movie_set_meta(_id, self.meta_user_info)
			if not meta or 'blank_entry' in meta: return
			meta_get = meta.get
	...
	def movie_sets_worker(self):
		self.meta_user_info, self.current_date, self.adjust_hours = metadata_user_info(), get_datetime_function(), date_offset()
		self.fanart_enabled = self.meta_user_info['extra_fanart_enabled']
