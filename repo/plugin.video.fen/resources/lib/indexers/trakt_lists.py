# -*- coding: utf-8 -*-
from apis.trakt_api import trakt_fetch_collection_watchlist, get_trakt_list_contents, trakt_trending_popular_lists, trakt_get_lists, trakt_search_lists, trakt_fetch_movie_sets
...
def build_trakt_movie_sets(params):
	...
	try:
		...
		# if paginate(): process_list, all_pages, total_pages = paginate_list(collection_info, page_no, page_limit())
		# else: process_list, all_pages, total_pages = collection_info, [], 1
		# if total_pages > 2 and not is_widget:
		# 		page_ref = page_reference()
		# 		if page_ref != 3:
		# 			add_dir({'mode': 'navigate_to_page_choice', 'media_type': 'Media', 'current_page': page_no, 'total_pages': total_pages, 'all_pages': all_pages,
		# 					'transfer_mode': 'trakt.list.build_trakt_movie_sets', 'page_reference': page_ref}, jump2_str, handle, 'item_jump', isFolder=False)
		add_items(handle, Movies({'list': [i['id'] for i in collection_info]}).movie_sets_worker())
		# if total_pages > page_no:
		# 	new_page = str(page_no + 1)
		# 	add_dir({'mode': 'trakt.list.build_trakt_movie_sets', 'new_page': new_page}, nextpage_str % new_page, handle, 'item_next')
	except: pass
	set_content(handle, content)
	end_directory(handle, False if is_widget else None)
	if not is_widget: set_view_mode('view.%s' % content, content)
