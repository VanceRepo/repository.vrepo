...
personal = {'in_progress_tvshows': ('modules.watched_status', 'get_in_progress_tvshows'), 'favourites_tvshows': ('modules.favourites', 'get_favourites'),
	...
	def build_tvshow_content(self, item_position, _id):
		try:
			...
			set_properties({'watchedepisodes': string(total_watched), 'unwatchedepisodes': string(total_unwatched), 'watchedprogress': string(progress),
							'totalepisodes': string(total_aired_eps), 'totalseasons': string(total_seasons), 'fen_sort_order': string(item_position),
							'fen_playcount': string(playcount), 'fen_extras_params': extras_params, 'fen_options_params': options_params})
