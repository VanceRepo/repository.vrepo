...
def movie_meta(id_type, media_id, user_info, current_date):
	...
	meta, fanarttv_data, custom_artwork = metacache_get('movie', id_type, media_id)
	if meta:
	...
	try:
		...
		if not data: return None
			...
			metacache_set('movie', id_type, meta, EXPIRES_6_HOURS)
			return meta
		if language != 'en':
		...
		fanart_data = fanarttv_data or default_fanarttv_data
		meta.update(fanart_data)
		meta.update(custom_artwork)
		metacache_set('movie', id_type, meta, movie_expiry(current_date, meta))
	except: pass
	return meta
...
def tvshow_meta(id_type, media_id, user_info, current_date):
	...
	if media_id == None: return None
	language, extra_fanart_enabled, fanart_client_key = user_info['language'], user_info['extra_fanart_enabled'], user_info['fanart_client_key']
	meta, fanarttv_data, custom_artwork = metacache_get('tvshow', id_type, media_id)
	if meta:
		if not meta.get('fanart_added', False) and extra_fanart_enabled:
	...
	try:
		...
		if not data: return None
			...
			metacache_set('tvshow', id_type, meta, EXPIRES_6_HOURS)
			return meta
		if language != 'en':
			if data['overview'] in empty_value_check:
				media_id, id_type = data['id'], 'tmdb_id'
...
def movie_set_meta(media_id, user_info):
	if media_id == None: return None
	id_type = 'tmdb_id'
	extra_fanart_enabled, fanart_client_key = user_info['extra_fanart_enabled'], user_info['fanart_client_key']
	meta, fanarttv_data, custom_artwork = metacache_get('movie_set', id_type, media_id)
	if meta:
		if not meta.get('fanart_added', False) and extra_fanart_enabled:
			fanart_data = fanarttv_get('movies', '', meta['tmdb_id'], fanart_client_key)
			meta.update(fanart_data)
			metacache_set('movie_set', id_type, meta, EXPIRES_30_DAYS)
		return meta
	try:
		tmdb_api = user_info['tmdb_api']
		...
		if not data: return None
			...
			metacache_set('movie_set', id_type, meta, EXPIRES_6_HOURS)
			return meta
		if not fanarttv_data and extra_fanart_enabled: fanarttv_data = fanarttv_get('movies', '', data['id'], fanart_client_key)
		image_resolution = user_info.get('image_resolution', backup_resolutions)
		data_get = data.get
...
def season_episodes_meta(season, meta, user_info):
	...
	data = metacache_get('season', 'tmdb_id', string)[0]
	if data: return data
	try:
