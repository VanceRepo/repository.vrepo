...
set_property, clear_property, convert_language, get_visibility = ku.set_property, ku.clear_property, ku.convert_language, ku.get_visibility
...
class FenPlayer(xbmc_player):
	def __init__ (self):
		...
		self.media_marked, self.subs_searched, self.nextep_info_gathered, self.nextep_started, self.random_continual_started = False, False, False, False, False
		self.autoplay_next_episode, self.play_random_continual, self.failed_playback, self.return_value = False, False, False, True
		...
		self.disable_content_lookup = disable_content_lookup()

	def run(self, url=None, media_info=None):
		close_all_dialog()
		if not url: return
	...
	def monitor(self):
		...
		if not self.check_playback_start(): return
		sleep(1000)
		while self.isPlayingVideo():
			try:
				...
				if self.current_point >= self.set_watched and not self.media_marked:
					...
					if self.play_random_continual and not self.random_continual_started: self.run_random_continual()
	...
	def media_watched_marker(self):
		...
		try:
			if self.current_point >= self.set_watched:
				...
				clear_property('fen_random_episode_history')
	...
	def run_next_ep(self):
		...
		try: Thread(target=execute_nextep, args=(self.meta, self.nextep_settings)).start()
		except: pass

	def run_random_continual(self):
		self.random_continual_started = True
		from modules.episode_tools import play_random_continual
		try: Thread(target=play_random_continual, args=(self.tmdb_id,)).start()
		except: pass
	...
	def info_next_ep(self):
		...
		try:
			...
			self.start_prep = self.nextep_settings['scraper_time'] + window_time
			self.nextep_settings['start_prep'] = self.start_prep
		except: pass
