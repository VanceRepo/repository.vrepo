...
class Sources():
	...
	def play_source(self, results):
		if self.background: return self.play_file(results, autoplay=True, background=True)
	...
	def scrapers_dialog(self):
		def _scraperDialog():
			if not self.progress_dialog: self._make_progress_dialog(enable_fullscreen=True)
	...
	def _clear_properties(self):
		...
		if self.active_folders:
			for item in self.folder_info: clear_property('%s.internal_results' % item[0])

	def _make_progress_dialog(self, enable_fullscreen):
		self.progress_dialog = confirm_progress_media(meta=self.meta, enable_fullscreen=enable_fullscreen)
	...
	def play_file(self, results, source={}, autoplay=False, background=False):
		...
		def _resolver():
			self.url = None
			if not background: self._make_progress_dialog(enable_fullscreen=False)
		...
		try:
			...
			FenPlayer().run(self.url, self.meta)
		except: pass
