...
class source:
			...
			while not self.progress_dialog.iscanceled() or monitor.abortRequested():
				try:
					...
					if self.threads_completed:
						if len_alive_threads == 0 or percent >= 100: close_dialog = False; break
	...
	def process_duplicates(self):
		def _process(sources):
			...
			uniqueURLs_add, uniqueHashes_add = uniqueURLs.add, uniqueHashes.add
					...
					if url not in uniqueURLs:
						...
						if 'hash' in provider:
							if provider['hash'] not in uniqueHashes:
								uniqueHashes_add(provider['hash'])
								yield provider
						else: yield provider
