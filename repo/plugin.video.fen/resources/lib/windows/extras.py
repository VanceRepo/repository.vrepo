...
from metadata import movie_set_meta
...
fetch_kodi_imagecache, addon_fanart, empty_poster, backup_cast_thumbnail = kodi_utils.fetch_kodi_imagecache, kodi_utils.addon_fanart, kodi_utils.empty_poster, get_icon('genre_family')
...
class Extras(BaseDialog):
	...
	def onInit(self):
		tasks = (self.set_poster, self.make_cast, self.make_recommended, self.make_reviews, self.make_trivia, self.make_blunders, self.make_parentsguide,
	...
	def onAction(self, action):
		...
		if action in self.context_actions:
			...
			if focus_id in (posters_id, backdrops_id):
				...
				params = {'action': 'image', 'name': '%s %s' % (self.rootname, chosen_listitem.getProperty('name')),
		...
		if action in self.selection_actions:
			...
			if self.control_id == cast_id:
				...
				end_index = Images().run({'mode': 'slideshow_image', 'all_images': self.get_attribute(self, chosen_var), 'current_index': self.get_position(self.control_id)})
				self.getControl(self.control_id).selectItem(end_index)
			elif self.control_id == videos_id:
				chosen = dialogs.imdb_videos_choice(self.get_attribute(self, chosen_var)[self.get_position(self.control_id)]['videos'], self.poster)
	...
	def make_posters(self):
		self.make_artwork('posters')

	def make_fanart(self):
		self.make_artwork('backdrops')
	...
	def make_artwork(self, image_type):
		...
		def builder():
			for count, item in enumerate(data, 1):
				try:
					...
					listitem.setProperty('name', '%02d. %s' % (count, self.title))
					listitem.setProperty('thumbnail', thumb_url)
		...
		try:
			...
			tmdb_image_list.extend([(tmdb_image_base % ('original', i['file_path']), self.title) for i in data])
			if used_image != default_image and 'image.tmdb' in used_image:
				try:
	...
	def make_collection(self):
		...
		try:
			data = movie_set_meta(coll_id, settings.metadata_user_info())
	...
	def original_poster(self):
		poster = self.meta.get('custom_poster') or self.meta.get(self.poster_main) or self.meta.get(self.poster_backup) or empty_poster
	...
	def original_fanart(self):
		fanart = self.meta.get('custom_fanart') or self.meta.get(self.fanart_main) or self.meta.get(self.fanart_backup) or addon_fanart
		return fanart
	...
	def set_starting_constants(self, kwargs):
		...
		self.rating = str(round(self.meta['rating'], 1))
